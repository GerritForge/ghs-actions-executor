...
LINK_EXECUTOR_SOURCE=ln -s /home/jenkins/actions-executor /home/jenkins/gerrit/plugins

ACTIONS_EXECUTOR_BUILD=${GERRIT_CLONE} && ${LINK_EXECUTOR_SOURCE} && cd /home/jenkins/gerrit && \
	. set-java.sh 17 && \
	bazelisk build //plugins/actions-executor:actions-executor_deploy.jar && cp -r bazel-bin/plugins/actions-executor/actions-executor_deploy.jar /home/jenkins/actions-executor/output/ && \
	shasum bazel-bin/plugins/actions-executor/actions-executor_deploy.jar

TARGET=output/actions-executor_deploy.jar
...
${TARGET}: Makefile BUILD ${SRCS}
	...
	docker run --rm -v $(shell pwd):/home/jenkins/actions-executor -u jenkins -ti $(BUILD_IMAGE) bash -c '${ACTIONS_EXECUTOR_BUILD}'
