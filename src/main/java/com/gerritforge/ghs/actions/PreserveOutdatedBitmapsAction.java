...
import static com.gerritforge.ghs.actions.BitmapGenerationLog.GHS_PACKS_LOG;
...
class PreserveOutdatedBitmapsAction implements Action {
      ...
      snapshot.ifPresentOrElse(
          snapshotPath -> {
            try {
              ...
              logger.atInfo().log(
                  "processed %d files in %s repository", info.files(), repositoryPath);
              Files.deleteIfExists(snapshotPath);
              if (info.last.isPresent()) {
                keepMostRecentBitmapInLog(info.last.get(), repositoryPath);
              } else {
  ...
  private static void keepMostRecentBitmapInLog(ObjectId last, String repositoryPath)
      throws IOException {
  ...
  @Nullable
  private static Path getMostRecentExistingBitmap(Path packsDir) throws IOException {
    Path latest = null;
  ...
  private PreserveInfo preserveOldPacks(
      Path preservedDir, Path packsDir, List<ObjectId> packEntriesInLog) throws IOException {
    if (packEntriesInLog.isEmpty()) {
      logger.atFine().log(
          "No entries found in %s. Nothing to preserve.", packsDir.resolve(GHS_PACKS_LOG));
      return EMPTY;
    }
    ...
    for (ObjectId packId : packEntriesInLog) {
      ...
      Path packBitmapIndexPath =
          packsDir.resolve(String.format("%s.%s", packName, BITMAP_INDEX.getExtension()));

      if (mostRecentBitmap != null
          && packBitmapIndexPath.getFileName().equals(mostRecentBitmap.getFileName())) {
        logger.atInfo().log(
