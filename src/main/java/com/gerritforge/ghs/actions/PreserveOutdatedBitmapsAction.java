...
class PreserveOutdatedBitmapsAction implements Action {
  ...
  private static final String PRUNE_PACK_EXPIRE_DEFAULT = PRUNE_PACK_EXPIRE_SECONDS_DEFAULT + ".seconds.ago";
      ...
      snapshot.ifPresentOrElse(
          snapshotPath -> {
            try {
              ...
              PreserveInfo info = preserveOldPacks(preservedDir, packsDir, entriesFromLog, packfilePruneCutoff);
              logger.atInfo().log(
  ...
  private Instant getPrunePackExpire(String repositoryPath) {
    try {
      try (FileRepository repository = new FileRepository(repositoryPath)) {
        ...
        return GitTimeParser.parseInstant(MoreObjects.firstNonNull(prunePackExpire, PRUNE_PACK_EXPIRE_DEFAULT));
      }
    } catch (IOException | ParseException e) {
      logger.atSevere().withCause(e).log("Unable read gc.prunePackExpire from Git config: defaulting to %s", PRUNE_PACK_EXPIRE_DEFAULT);
      return SystemReader.getInstance().civilNow().atZone(SystemReader.getInstance().getTimeZoneId())
              .toInstant().minusSeconds(PRUNE_PACK_EXPIRE_SECONDS_DEFAULT);
    }
  }
  ...
  private PreserveInfo preserveOldPacks(
      Path preservedDir, Path packsDir, Set<ObjectId> packEntriesInLog, Instant packfilePruneCutoff) throws IOException {
    ...
    for (ObjectId packId : packEntriesInLog) {
      ...
      if (packBitmapSnapshot.lastModifiedInstant().isAfter(packfilePruneCutoff)) {
        logger.atInfo().log(
                "%s has not expired. Do not move to preserved.", packName);
        packsToKeepInLog.add(packId);
        continue;
      }
  ...
  private static long movePacksToPreserved(Path packsDir, Path preservedDir, ObjectId packId)
    ...
    for (PackExt ext : List.of(BITMAP_INDEX, INDEX, PACK)) {
      PackFile packFile = new PackFile(packsDir.toFile(), "pack-" + packId.getName() + "." + ext.getExtension());
      if (packFile.getAbsoluteFile().exists()) {
        PackFile oldPackFile = packFile
                .createPreservedForDirectory(preservedDir.toFile());
